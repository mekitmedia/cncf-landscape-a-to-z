...
description: Multi-harness skill to run the CNCF weekly Editor → Researcher → Writer workflow with batching, subagent research delegation, and ground truth verification.
...
## Purpose
Use this skill in Claude Code, Codex/OpenAI, OpenCode, or other harnesses to perform the CNCF weekly Editor → Researcher → Writer workflow with strict batching, deep subagent-driven research, and verifiable ground-truth links.
...
2. **Invoke a `research` subagent** (or run targeted searches) specifically for the project.
...
2. Write or update `website/content/posts/<YEAR>-<WEEK_LETTER>.md` with frontmatter:
...
- Update `data/weeks/<WEEK_ID>/tracker.yaml`:
  ...
  - If a project research task fails, set status to `failed` with error details.
